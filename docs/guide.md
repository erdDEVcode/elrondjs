...
* PEM file
* JSON keyfile
* _Ledger - coming soon!_

The ElrondWallet class provides static methods for loading in each of these types, for example:

```js
const { ElrondWallet } = require('elrondjs')

const wallet = ElrondWallet.fromMenmonic('tourist judge garden detail summer differ want voyage foot good design text')
```

Creating a "dummy" wallet with a random mnemonic can be done using the generateMenmonic function:

```js
const { ElrondWallet, generateMnemonic } = require('elrondjs')

const wallet = ElrondWallet.fromMenmonic(generateMnemonic())
```

Wallets are primarily used to sign transactions (see below).
...
To sign a transaction we need a Signer instance. The ElrondWallet class implements the Signer interface and thus an instance of it can be used to sign a Transaction:
...
const contract = await Contract.at('contract bech32 address here', {
  provider: ...// Provider instance,
  signer: ...// Singer instance, e.g. an ElrondWallet
...
1. Broadcast the signed transaction to the network
